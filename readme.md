...
LitheLLM 是一个轻量、灵活且高效的大语言模型（LLM）分布式训练框架。本项目支持从**预训练（Pretraining）**、**指令微调（SFT）**到**强化学习对齐（GRPO）**的完整训练链路。同时支持 NVIDIA GPU 生态和华为 NPU（Ascend）环境。
...
├── dataset/              # 数据集及数据处理逻辑
│   ├── full_sft.jsonl    # SFT 样例数据
│   ├── rlaif.jsonl       # GRPO 样例数据
...
SFT 阶段会自动调用 `apply_chat_template` 并且只对 Assistant 的回复计算 Loss。数据集应为标准的对话格式（支持 tools 和 reasoning_content）。
